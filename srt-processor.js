...
function processSegments(segments, variables, errors, warnings) {
  ...
  return segments.map((segment, index) => {
    ...
    // Timecode validation
    const timecodeRegex = /\d{1,2}:\d{1,2}:\d{1,2},\d{3} --> \d{1,2}:\d{1,2}:\d{1,2},\d{3}/;
      ...
      if (singleTimecodeRegex.test(timecodeLine.trim())) {
        if (index < segments.length - 1) {
          ...
          if (nextStartTime && timecodeRegex.test(nextSegmentLines[1])) {
            const prevEndTime = lastTimecodeLine ? lastTimecodeLine.split(' --> ')[1] : '00:00:00,000';
            timecodeLine = `${prevEndTime} --> ${nextStartTime.trim()}`;
            warnings.push(`Segment ${newIndex}: Missing start or end time. Original: "${originalTimecodeLine}". Corrected to: "${timecodeLine}"`);
          } else {
            errors.push(`Segment ${newIndex}: Invalid timecode "${originalTimecodeLine}" and the next segment's timecode is also invalid.`);
          }
        } else {
          errors.push(`Segment ${newIndex}: Missing an end time and it's the last segment. Timecode: "${originalTimecodeLine}"`);
        }
      } else {
        errors.push(`Segment ${newIndex}: Invalid timecode format. Timecode: "${originalTimecodeLine}"`);
      }
    }
    ...
    if (startTime && endTime) {
      ...
      if (startTimeMs >= endTimeMs) {
        errors.push(`Segment ${newIndex}: Invalid timecode (start >= end). Timecode: "${timecodeLine}"`);
      }

      if (lastTimecodeLine) {
        const prevEndTimeMs = timecodeToMilliseconds(lastTimecodeLine.split(' --> ')[1]);
        if (prevEndTimeMs > startTimeMs) {
          warnings.push(`Segment ${newIndex}: Overlaps with the previous segment. Previous end time: ${lastTimecodeLine.split(' --> ')[1]}, current start time: ${startTime}`);
        }
      }
    }
...
function processSrt(data, variables = null) {
  ...
  const processedSegments = processSegments(filteredSegments, variables, errors, warnings);

  if (variables) {
    processedSegments.forEach(segment => {
      const newContent = segment.content.replace(/{{([^}]+)}}/g, (match, variableName) => {
  ...
  const outputContent = processedSegments.map(segment => `${segment.index}\n${segment.timecode}\n${segment.content}`).join('\n\n');
